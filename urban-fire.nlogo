...
to setup
  ...
  ;let tree-map-filename "/home/zvi/Desktop/0_trees_fire_propagation/9-these-maps-show-every-tree-in-a-city.jpg"
  let tree-map-filename "data/nyc_tree_map.jpeg"
  let tree-map-dx 686
  let tree-map-dy 386

  __change-topology false false
  set-patch-size 1
  resize-world 0 (tree-map-dx - 1) 0 (tree-map-dy - 1)
  import-pcolors tree-map-filename
  ...
  ; binarize - only green and white
  ask patches with [pcolor > white ] [set pcolor green]
  ask patches with [pcolor <= white] [set pcolor white]

  ;ask patches with [pcolor = green] [ ask neighbors4 [set pcolor green] ]
  ;ask patches with [pcolor = green] [ ask neighbors [set pcolor one-of [green green white] ] ]
  ;ask patches with [pcolor < white] [set pcolor green]
